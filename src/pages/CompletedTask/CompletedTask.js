...
const CompletedTask = () => {
    ...
    const { data: tasks, isLoading, refetch } = useQuery({
        queryKey: ['completed'], queryFn: async () => {
            const response = await fetch(`https://g-task-server.vercel.app/completed?userid=${user.uid}`);
    ...
    return (
        <div className='my-5'>
            <h2 className='text-center text-2xl font-semibold mb-5 dark:text-white'>Completed Tasks</h2>
            ...
            <div className='flex flex-wrap gap-y-5'>
                <div className='w-full md:w-1/2 lg:w-1/3 flex flex-col gap-5 px-2'>
                    {
                        tasks_column_1.map(task => <CompletedTaskCard
                            key={task._id}
                            task={task}
                            handleTaskDelete={handleTaskDelete}
                            refetch={refetch}
                ...
                <div className='w-full md:w-1/2 lg:w-1/3 flex flex-col gap-5 px-2'>
                    {
                        tasks_column_2.map(task => <CompletedTaskCard
                            key={task._id}
                            task={task}
                            handleTaskDelete={handleTaskDelete}
                            refetch={refetch}
                ...
                <div className='w-full md:w-1/2 lg:w-1/3 flex flex-col gap-5 px-2'>
                    {
                        tasks_column_3.map(task => <CompletedTaskCard
                            key={task._id}
                            task={task}
                            handleTaskDelete={handleTaskDelete}
                            refetch={refetch}
